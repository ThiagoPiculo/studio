...
import { addMissionTemplate, getMissionTemplatesByOwnerOrFamily } from '@/lib/supabase/db';
...
export function ShareMissionDialog({ template, isOpen, onOpenChange, onShared }: ShareMissionDialogProps) {
  const { user } = useAuth();
    ...
    if (isOpen && user && template) {
        ...
        const fetchAllTemplates = async () => {
            try {
                ...
                toast({ title: "Erro ao verificar missões", variant: "destructive" });
            } finally {
  ...
  const handleShare = async () => {
    ...
    try {
      const templateData: Omit<MissionTemplate, 'id' | 'createdAt' | 'updatedAt' | 'status' | 'familyId'> = {
        ownerId: user.uid,
        title: template.title,
        description: template.description,
        emoji: template.emoji,
        category: template.category,
        starsReward: template.starsReward,
      ...
      await addMissionTemplate(user, templateData, selectedContexts);
      ...
      console.error("Error sharing mission template:", error);
      toast({ title: "Erro ao Compartilhar", description: "Não foi possível copiar a missão.", variant: "destructive" });
    } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-xl">Compartilhar Missão</DialogTitle>
          <DialogDescription>
            Copie a missão "<span className="font-semibold text-primary">{template.title}</span>" para outros espaços de trabalho.
          </DialogDescription>
        </DialogHeader>
        ...
        {isLoadingData ? (
            ...
            <div className="py-6 text-center text-muted-foreground">
                Você não tem outros espaços de trabalho para compartilhar esta missão.
            </div>
        ) : (
