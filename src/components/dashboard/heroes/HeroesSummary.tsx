...
export function HeroesSummary() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
        ...
        const [childrenData, missionData, rewardData, scheduleData] = await Promise.all([
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {filteredChildren.map((child) => {
              const age = calculateAge(child.birthDate as any);
              ...
              const todaysMissions: MissionInstance[] = missionInstances
                ...
                .sort((a, b) => {
                    const timeA = getDateObject(a.startDate) || getDateObject(a.dueDate);
                    const timeB = getDateObject(b.startDate) || getDateObject(b.dueDate);
                            ...
                            <div className="h-auto w-full mt-2">
                                ...
                                {todaysMissions.length > 0 ? (
                                  missionsToShow.map(item => {
                                    ...
                                    const eventTime = getDateObject(item.startDate) || getDateObject(item.dueDate);
